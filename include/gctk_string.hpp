...
namespace Gctk {
	...
	public:
		...
		inline char* data() { return m_sString.data(); }
		[[nodiscard]] inline const char* data() const { return m_sString.data(); }
		[[nodiscard]] inline const char* c_str() const { return m_sString.c_str(); }
...
template<>
struct fmt::formatter<Gctk::String> : fmt::formatter<std::string_view> {
	auto format(Gctk::String str, fmt::format_context& ctx) const {
